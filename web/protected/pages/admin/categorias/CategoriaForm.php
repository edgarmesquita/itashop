...
class CategoriaForm extends TPage
{
    ...
    public function onInit($param)
    {
        ...
        switch($this->Request['action'])
        {
            case 'view' :
                ...
                $this->vDescricao->Text = $item->descricao;
            ...
            case 'edit' :
                ...
                $this->fDescricao->Text = $item->descricao;
    ...
    protected function getPost()
    {
        ...
        $record = CategoriaRecord::finder()->findByPk($id);
        if($record === null)
    ...
    public function salvarClick($sender, $param)
    {
        if($this->IsValid) // when all validations succeed
        {
            if($this->Request['action'] == 'edit')
                ...
                $item = new CategoriaRecord();

            $item->nome = $this->fNome->Text;
            $item->descricao = $this->fDescricao->Text;
            $item->posicao = $this->fPosicao->Text;
            $item->ativo = $this->fAtivo->SelectedValue;
            $item->save();

            $dir = "assets/data/produtos/" . $item->categoria;
            if(!file_exists($dir))
                mkdir($dir);

            if(empty($item->categoria)) $url = $this->Service->constructUrl('admin.categorias.CategoriaList');
            else $url = $this->Service->constructUrl('admin.categorias.CategoriaForm', array('action' => 'view', 'id' => $item->categoria));
